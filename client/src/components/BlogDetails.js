...
const BlogDetails = () => {
  const { id } = useParams(); // Fetched from <Route path="/blogs/:id" component={BlogDetails} /> from App.js
  ...
  useEffect(() => {
    ...
    fetchBlog();
  }, [id]); // This dependancy array re-run only if the id changes to a different value after the initial fetch.
